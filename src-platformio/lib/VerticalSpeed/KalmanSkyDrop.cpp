...
KalmanSkyDrop::KalmanSkyDrop(Sensor_MS5611 & _baro) 
	: Task("SkyDrop", 2 * 1024, 3)
	, baro(_baro)
	, seaLevel(1013.25)
	, mTimer(NULL)
...
KalmanSkyDrop * KalmanSkyDrop::mActiveVario = NULL;


int KalmanSkyDrop::begin(float Q_accel, float R_altitude)
{
	//
	baro.begin();
	...
	baroAltitude = Sensor_MS5611::getAltitude(baro.getPressure(), seaLevel);
	altitudeDrift = 0.0;

	KalmanFilter::Configure(Q_accel, R_altitude, baroAltitude);
...
void IRAM_ATTR KalmanSkyDrop::TimerProc() 
{
	if (KalmanSkyDrop::mActiveVario != NULL)
	{
		BaseType_t xHigherPriorityTaskWoken = pdFALSE;
		xSemaphoreGiveFromISR(KalmanSkyDrop::mActiveVario->mSemaphore, &xHigherPriorityTaskWoken);
		if( xHigherPriorityTaskWoken == pdTRUE) 
			portYIELD_FROM_ISR(); // this wakes up task immediately instead of on next FreeRTOS tick
	}
}

void KalmanSkyDrop::TaskProc() 
{
	while (1)
	{
		//
...
int	KalmanSkyDrop::available()
{
	return varioUpdated;
}

void KalmanSkyDrop::flush()
{
	varioUpdated = false;
}

void KalmanSkyDrop::update()
{
	//if (baro.available())
	{
		...
		//
		float rawAltitude = Sensor_MS5611::getAltitude(prs, seaLevel);
		baroAltitude += (rawAltitude - baroAltitude) * 0.1;
...
void KalmanSkyDrop::calculateSeaLevel(float altitude)
{
